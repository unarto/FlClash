...
func (s *System) tunLoop() {
	...
	for {
		...
		if len(packet) > 0 && s.debugPacketCount < 8 {
			s.debugPacketCount++
			head := len(packet)
			if head > 16 {
				head = 16
			...
			s.logger.Info(
				"[sing-tun] System.tunLoop read len=",
				len(packet),
				" version=",
				header.IPVersion(packet),
				" head=",
				packet[:head],
			)
		}
	...
	for {
		...
		for i := 0; i < n; i++ {
			...
			if len(packet) > 0 && s.debugPacketCount < 8 {
				...
				s.logger.Info(
					"[sing-tun] System.batchLoopLinux read len=",
					len(packet),
					" version=",
					header.IPVersion(packet),
					" head=",
...
func (s *System) processPacket(packet []byte) bool {
	...
	case header.IPv4Version:
		if len(packet) > 0 && s.debugPacketCount < 64 {
			s.debugPacketCount++
			ipHdr := header.IPv4(packet)
			s.logger.Info(
				fmt.Sprintf(
					"[sing-tun] System.processPacket ipv4 proto=%d src=%s dst=%s len=%d",
					ipHdr.TransportProtocol(),
					ipHdr.SourceAddr().String(),
					ipHdr.DestinationAddr().String(),
					len(packet),
				),
			)
		}
	...
	case header.IPv6Version:
		if len(packet) > 0 && s.debugPacketCount < 64 {
			...
			s.logger.Info(
				fmt.Sprintf(
					"[sing-tun] System.processPacket ipv6 proto=%d src=%s dst=%s len=%d",
					ipHdr.TransportProtocol(),
					ipHdr.SourceAddr().String(),
					ipHdr.DestinationAddr().String(),
					len(packet),
...
func (s *System) processIPv4UDP(ipHdr header.IPv4, udpHdr header.UDP) error {
	...
	if destination.Port() == 53 {
		s.logger.Info(
			fmt.Sprintf(
				"[sing-tun] System.processIPv4UDP source=%s destination=%s payload=%d",
				source.String(),
				destination.String(),
				data.Len(),
			),
...
func (s *System) processIPv6UDP(ipHdr header.IPv6, udpHdr header.UDP) error {
	...
	if destination.Port() == 53 {
		s.logger.Info(
			fmt.Sprintf(
				"[sing-tun] System.processIPv6UDP source=%s destination=%s payload=%d",
				source.String(),
				destination.String(),
				data.Len(),
			),
