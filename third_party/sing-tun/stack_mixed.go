...
func (m *Mixed) processPacket(packet []byte) bool {
	...
	case header.IPv4Version:
		if len(packet) > 0 && m.debugPacketCount < 64 {
			m.debugPacketCount++
			ipHdr := header.IPv4(packet)
			m.logger.Info(
				fmt.Sprintf(
					"[sing-tun] Mixed.processPacket ipv4 proto=%d src=%s dst=%s len=%d",
	...
	case header.IPv6Version:
		if len(packet) > 0 && m.debugPacketCount < 64 {
			...
			m.logger.Info(
				fmt.Sprintf(
					"[sing-tun] Mixed.processPacket ipv6 proto=%d src=%s dst=%s len=%d",
					ipHdr.TransportProtocol(),
					ipHdr.SourceAddr().String(),
					ipHdr.DestinationAddr().String(),
					len(packet),
