...
func (t *NativeTun) Read(p []byte) (n int, err error) {
	if t.vnetHdr {
		...
		if err == nil && n > 0 && t.debugReadCount < 8 {
			t.debugReadCount++
			head := n
			if head > 16 {
				head = 16
			}
			log.Printf("[sing-tun] Read fd=%d len=%d head=% x", t.tunFd, n, p[:head])
		}
		return
	}
}
...
func (t *NativeTun) BatchRead(buffers [][]byte, offset int, readN []int) (n int, err error) {
	...
	if n > 0 && t.debugReadCount < 8 {
		t.debugReadCount++
		head := n
		if head > 16 {
			head = 16
		}
		log.Printf("[sing-tun] BatchRead fd=%d len=%d head=% x", t.tunFd, n, t.writeBuffer[:head])
	}
