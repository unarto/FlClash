...
func (f *UDPForwarder) HandlePacket(id stack.TransportEndpointID, pkt *stack.PacketBuffer) bool {
	...
	if upstreamMetadata.Destination.Port == 53 {
		log.Infoln("[sing-tun] GVisor.HandlePacket source=%s destination=%s",
			upstreamMetadata.Source.AddrPort().String(),
			upstreamMetadata.Destination.AddrPort().String(),
		)
	}
